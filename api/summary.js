...
export async function GET(req) {
 ...
 try {
  ...
  const messages=await getMessages(...range), text=await summarize(messages,date);
 ...
 } catch(e) {console.error('Summary failed'); return json({error:['DELIVERY_REQUIRES_REVIEW','GEMINI_NOT_CONFIGURED','GEMINI_INVALID_MODEL','GEMINI_INPUT_TOO_LARGE','GEMINI_CONNECTION_FAILED','GEMINI_RATE_LIMITED','GEMINI_REQUEST_FAILED','GEMINI_INVALID_RESPONSE','GEMINI_INCOMPLETE_RESPONSE','GEMINI_OUTPUT_TOO_LONG'].includes(e.message)?e.message:'SUMMARY_UNAVAILABLE'},503);}
}
